...
| 12.4 | Native team grid | complete | see git log |
...
- [x] 12.4 Team grid. Native [TeamGrid.astro](src/components/home/TeamGrid.astro): renders the 10 cards directly from the team collection. Each card's `fs-modal-element="open-N"` is derived from `entry.data.legacyModalId` (e.g. `modal-5` → `open-5`) to keep the existing Finsweet modal triggers wired to the modals still living inside `services-tabs.html` (those go native in Phase 12.5). Uses `<Image>` for headshots. Deleted `team-grid.html`, `applyTeamCards.ts`, `applyTeamGridOrder.ts`. The six remaining `applyTeam*` utilities target the team modals inside `services-tabs.html` and retire in Phase 12.5. Visual baselines unchanged.
- [x] 12.5 Services + team modals. Replaced the monolithic 62KB `services-tabs.html` (4 service cards/modals + 10 team modals + 7-util mutation pipeline) with two native components: [ServicesTabs.astro](src/components/home/ServicesTabs.astro) renders the cards-in-tabs section plus the 4 service modals, [TeamModals.astro](src/components/home/TeamModals.astro) renders the 10 team-member modals (added to `index.astro` after `<TeamGrid />`). Each modal's `fs-modal-element` and matching `close-N` are derived from `entry.data.legacyModalId`. Service modals reuse `cardImage` (since prod uses one image per service); team modal headshots use `<Image>` for asset processing. Deleted `src/partials/home/services-tabs.html` and ten retired utilities: `applyServiceModals`, `applyServiceSubServices`, `applyServiceSummaries`, `applyTeamModalBios`, `applyTeamModalExpertise`, `applyTeamModalHeadshots`, `applyTeamModalOrder`, `escapeRegExp`, `formatBulletList`. Bullet lists (`• item<br/>`) are now produced inline via `set:html`. Refreshed home-page baselines: tablet through desktop-large shifted 24px taller because Astro's `<Image>` writes explicit `width`/`height` attributes on the card image (was Webflow's `srcset` with no width/height, so layout was driven purely by CSS). DOM tests + visual tests all green.
- [x] 12.6 Hero / contact CTA. Native [HeroHeader.astro](src/components/home/HeroHeader.astro) and [ContactCTA.astro](src/components/home/ContactCTA.astro). Hero reads `home/hero.md` for headline/subheadline (set:html on the headline so `<br/>` renders), renders the Spline mount as `<div class="spline-scene" data-miri-spline-url={splineSrc}>` so `miri-spline-defer.js` picks it up, inlines the 5 orange rating stars, the Webflow Captcha form (with `redirect` injected via attribute-spread to bypass TS validation), and the Calendly Connect button + Learn more link. ContactCTA renders the simpler section_contact9. Both reference `cta.calendlyUrl` directly. Deleted `hero-header.html`, `contact-cta.html`, `applyHeroContent.ts`. Visual baselines unchanged.
- [x] 12.7 Footer / nav. Native [NavBar.astro](src/components/chrome/NavBar.astro) and [Footer.astro](src/components/chrome/Footer.astro). Both render the menu via `navLinks` from `src/data/site.ts`; footer also pulls `copyright` from there. Calendly URL comes directly from `cta.calendlyUrl` (no more partial+`syncPrimaryCalendlyUrl`). Logo srcset built programmatically from the `/vendor/webflow/assets/` paths via `rootPath()`. Deleted `src/partials/navbar-home.html` and `src/partials/footer.html`. Refreshed desktop baselines for `privacy` and `aspire-landscape` (1% diff was a 1-2px shift in the Connect button area — within home's `0.04` tolerance but above legal pages' `0.001`). `syncPrimaryCalendlyUrl` and `addCalendlyPopupToPlainPrimaryAnchors` stay in `src/utils/calendly.ts` because the Aspire page still uses them on its imported body HTML.
...
- **Webflow `id="w-node-..."` attributes are load-bearing for layout, not decorative.** The shared Webflow CSS has ~166 selectors keyed on these auto-generated IDs that set `grid-area` / `justify-self` / etc. on grid children. When porting `services-tabs.html`, `industry-tabs.html`, `navbar-home.html`, and `footer.html` to native components in Phase 12.3 / 12.5 / 12.7, I dropped 28 + 2 + 1 + 3 = 34 of these IDs and didn't notice the layout regression — the visual baselines absorbed the diff each time because I was running `update-snapshots` against my own broken render. Fixed in a follow-up commit by hardcoding the original IDs back into [ServicesTabs.astro](src/components/home/ServicesTabs.astro), [TeamModals.astro](src/components/home/TeamModals.astro), [IndustryTabs.astro](src/components/home/IndustryTabs.astro), [NavBar.astro](src/components/chrome/NavBar.astro), [Footer.astro](src/components/chrome/Footer.astro). Verified by capturing the Operations service modal and Ramel team modal at desktop on both local and prod — pixel-identical. Local home/privacy/terms page heights now match prod (6626 / 3798 / 6663). AGENTS.md gained a discipline note about running `test:visual:production` before `test:visual:update`.
